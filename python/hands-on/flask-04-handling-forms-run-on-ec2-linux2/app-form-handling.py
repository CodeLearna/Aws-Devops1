...
# Write a function named `greet` which uses template file named `greet.html` given under `templates` folder. it takes parameters from query string on URL. Assisgn that parameter to "user" variable. If it doesn't have any parameter, warning massage is raised

@app.route("/greet", methods = ["GET"])
def greet():
    if "user" in request.arg:
        usr = request.args["user"]
        return render_template("greet.html", user = usr)
    else:
        return render_template("greeting.html", user = "Send your user anme with 'user name parameter' ")
